...
HOSTED ON VERCEL, TEST-ONLY, RISK KNOWN AND ACCEPTED FOR THIS PURPOSE:
Vercel's own WebSocket docs state new connections are "not guaranteed to
reach the same Vercel Function instance." Since active_sessions lives in
this process's memory, a message landing on a DIFFERENT instance mid-
conversation would find an empty dict and silently lose that person's
Brain and history. This is a genuine, real risk, not a hypothetical —
kept anyway because this deployment is explicitly short-term/testing
only, per an explicit decision to accept it for that purpose. Watch the
health-check endpoint's instance_id (see below) to actually SEE this
...
# Generated once when this process starts (a "cold start" on Vercel).
# Different instances get different values — this is how you can SEE
# whether a request is hitting a warm, already-running instance or a
# fresh one Vercel just spun up, which is the actual mechanism behind
# the session-dropping risk described above.
...
def health_check():
    ...
    On Vercel specifically: this also returns a random instance_id,
    generated once per cold start. If you refresh this page several
    times and see the SAME instance_id, you're hitting the same warm
    instance. If it CHANGES, Vercel spun up a different instance for
    that request — which is the exact behavior that could silently drop
    a Brain mid-conversation over a WebSocket (see module docstring's
    Vercel warning). Watching this value is the cheapest way to actually
    see the risk happen, not just take the docs' word for it.
    """
    return {
        "status": "E.D.I.T.H. server is running",
